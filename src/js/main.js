...
const signinFormHtmlData = `<h1  class="heading4 head flex">Sign in to continue.</h1>
<form class="flex signup_form" action=""><div class="input_section"><h4 class="input_section_head">Email address</h4>
    <div class="input_section_ele_wrapper flex"><input class="input_section_ele" id="signup_email_input" name="email" placeholder="johncarber@example.com">
        <svg xmlns="http://www.w3.org/2000/svg" class="input_section_ele_icon input_icon" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-mail"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline>
    </svg></div><div class="input_message_box flex">
...
<div class="already_memeber_button flex subtitle_text">
<span> New to prolic ?<span> Signin</span></span>
</div>`;
...
document.getElementById('signin_button').addEventListener('click', () => { 
    ...
    dialogbox.putDialogBox(signinFormHtmlData);
    
    const signinEmailInput = new emailInputBox(document.getElementById('signin_email_input'));
    
    const signinPasswordInput = new passwordInputBox(document.getElementById('signin_password_input'));
    const signinForm = new FormBox(document.querySelector('.signin_form'), signinEmailInput, signinPasswordInput);
    signinForm.afterSubmit = () => {
        signinForm.SubmittingForm();
        const signinFormData = new FormData(signinForm.form);
        signinFormData.append('type','signin');
        localStorage.setItem('email',document.getElementById('signin_email_input').value);
        localStorage.setItem('password',document.getElementById('signin_password_input').value);
        
            fetch('./src/backend/database/LogIn.php', {
                method: 'POST',
                body: signinFormData
            }).then((response) => {
                response.text().then((text) => {
                   ...
                   sessionStorage.setItem("recentItems",response.recentItems);
                    switch (response.loginResult) {
                        case 'email_not_available':
                            ...
                            break;
                        case 'wrong_password':
                            signinPasswordInput.showError('given password is wrong');
                              signinForm.SubmittedForm();
                            break;
                        case 'loging_in':
                               location.assign("./src/sidePages/dashboard.php");
